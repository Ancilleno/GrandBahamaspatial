...
# example of logit model on frequency of occurrence by habitat
# just replace focal species in first line of code
spec.freq<-replace(SpecSamp$"Bahama Warbler",SpecSamp$"Bahama Warbler">1,1)
# check frequency counts by habitat to ensure logit models are applicable - should have > 30 occurrences
table(spec.freq,SpecSamp$Habitat)
# run logit model with binomial response variable
spec.m1<-glm(spec.freq~SpecSamp$Habitat,family="binomial")
summary(spec.m1)
# get null model and test overall habitat effect using likelihood ratio test
spec.null<-glm(spec.freq~1,family="binomial")
anova(spec.null,spec.m1,test="LRT")


#### Tom's edits continue here for spatial analysis of richness, diversity and dissimilarity

# All.dists <- as.matrix(dist(cbind(AllDiversity$LONGITUDE, AllDiversity$LATITUDE))) #Incorrect calculation of distance - need lat/long correction
# Use this instead
library(SpatialEpi)
