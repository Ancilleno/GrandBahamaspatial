...
####>>>>>Perhaps Locality is better wording as these matrices are by location not sample
## Add the species x sample matrix to the AllDiversity data
SpecSamp<-merge(AllDiversity,LocalitySps)
## Add the observer x location matrix to the AllDiversity data
ObservSamp<-merge(AllDiversity,LocalityObs)
# write.csv(ObservSamp,"ObservSamp.csv")  # these two matrices don't match exactly - the observer matrix had 5 duplicates line, I exported and created a new one
# write.csv(SpecSamp,"SpecSamp.csv")
...
cum_ind<-rc$individuals ## What does this do?
...
#I adjusted the text to sit on top of the lines
text(473,356,"Chao")
text(460,336,"Jackknife")
text(457,306,"Bootstrap")
...
### General linear model of species richness by habitat using a Poisson error distribution for species counts
...
plot(SpecSamp$TotalSurveys,SpecSamp$SpeciesRichness,log="xy",xlab="Number of Sampling Events",
     #Leno says these surveys are not all transect surveys, Sampling Events is better wording
...
# NMDS does not converge
 ord.m1<-metaMDS(SpecSampMatTrans,dist="bray",k=4,autotransform=FALSE,maxit=200)

# Use Distance-based redundancy analysis for ordination with habitat and numbers of surveys as predictor variables
ord.m2<-dbrda(SpecSampMatTrans~SpecSamp$Habitat+log(SpecSamp$TotalSurveys),dist="bray")
summary(ord.m2)
anova(ord.m2,by="term") # uses permuation test to get p-values for differences in species composition by habitat and sampling effort
...
arrows(0,0,-0.85,0.50,lwd=2)
text(-0.85,0.6,"Number of Surveys",cex=1.2)


# Indicator species analysis by habitat types
library(labdsv)

# species x sample data in data frame format
SpecSampData<-SpecSamp[,15:292]

indic_out<-indval(SpecSampData,SpecSamp$Habitat)
summary(indic_out)

#                                   cluster indicator_value probability
##The first six species are found in thick brush and coppice or dense canopies
#In my experience more active in the early morning or cooler hours of the evening except 
#in heavily shaded areas where their activity is more constant

# Cuban Emerald                      1          0.2163       0.044
# Gray Catbird                       1          0.1634       0.042
# Black-throated Blue Warbler        1          0.1552       0.009
# Ovenbird                           1          0.1411       0.050
# Black-throated Green Warbler       1          0.0968       0.033
# Worm-eating Warbler                1          0.0844       0.034

#These two species are extremely rare in the data set. I do not see how this is more 
#than a coincidence because of their happenstance cooccurrence
# White-throated Sparrow             2          0.0519       0.014
# Common Tern                        2          0.0492       0.024

# I would recognize these species as using an open understory forest or public area 
# where large trees border open areas and walkwaysthough they use different parts of the trees and paths 
# Pine Warbler                       3          0.3736       0.001
# Olive-capped Warbler               3          0.2423       0.001
# Bahama Warbler                     3          0.2005       0.001
# Black-faced Grassquit              3          0.1881       0.004
# Bahama Yellowthroat                3          0.1505       0.006
# Zenaida Dove                       3          0.1336       0.003
# Brown-headed Nuthatch              3          0.0580       0.045

#These species are all shore birds
# Piping Plover                      4          0.2168       0.004
# Sanderling                         4          0.1968       0.005
# Black-bellied Plover               4          0.1727       0.048
# Wilson's Plover                    4          0.1312       0.027
# 
# Sum of probabilities                 =  127.036 
# 
# Sum of Indicator Values              =  12.87 
# 
# Sum of Significant Indicator Values  =  3.02 
# 
...
#### Tom's edits continue here for spatial analysis of richness, diversity and dissimilarity

# All.dists <- as.matrix(dist(cbind(AllDiversity$LONGITUDE, AllDiversity$LATITUDE))) 
#Incorrect calculation of distance - need lat/long correction to account for curvature and create x-y coordinates
# Use this instead
library(SpatialEpi)

# matrix of latitude and longitude coordinates
longlatmat<-cbind(SpecSamp$LONGITUDE,SpecSamp$LATITUDE)
colnames(longlatmat)<-c("longitude","latitude")

# create matrix of x and y coordinates in km using latlong2grid function
xy_coords<-as.matrix(latlong2grid(longlatmat))

# matrix of pairwise euclidean distances
distmat<-as.matrix(dist(xy_coords,method="euclidean",diag=TRUE,upper=TRUE))


library(ncf) #use ncf package for Moran's I

# No strong spatial autocorrelations among species richness or survey efforts
# spatial autocorrelation of species richness and numbers of surveys
ac_spec_rich<-correlog(longlatmat[,1],longlatmat[,2],SpecSamp$SpeciesRichness,increment=10,latlon=TRUE)
ac_spec_rich
plot(ac_spec_rich, main = "Correlogram of Species Richness")
ac_no_surveys<-correlog(longlatmat[,1],longlatmat[,2],SpecSamp$TotalSurveys,increment=10,latlon=TRUE)
ac_no_surveys
plot(ac_no_surveys, main = "Correlogram of Number of Surveys")

# Spatial cross-correlation between species richness and total surveys
cc_rich_surveys<-correlog(longlatmat[,1],longlatmat[,2],SpecSamp$SpeciesRichness,SpecSamp$TotalSurveys,increment=10,latlon=TRUE)
cc_rich_surveys
plot(cc_rich_surveys)

# Shannon diversity of birds and observers
ac_shannon_birds<-correlog(longlatmat[,1],longlatmat[,2],SpecSamp$SpeciesShannonDiversity,increment=10,latlon=TRUE)
ac_shannon_obs<-correlog(longlatmat[,1],longlatmat[,2],SpecSamp$ObserverShannonDiversity,increment=10,latlon=TRUE)


# Mantel tests and correlograms - Use Bray-Curtis dissimilarity with quarter power transformations

# species dissimilarity x spatial distance
spec_bray<-as.matrix(vegdist(SpecSampMatTrans,method="bray",diag=TRUE))
mantel(distmat,spec_bray)
plot(distmat,spec_bray)

# observer dissimilarity x spatial distance
ObservSamp<-read.csv("ObservSamp.csv",header=TRUE)  # read in corrected observer matrix
ObservSampMat<-as.matrix(ObservSamp[16:295])
ObservSampMatTrans<-ObservSampMat^0.25

observ_bray<-as.matrix(vegdist(ObservSampMatTrans,method="bray",diag=TRUE))
mantel(distmat,observ_bray)
plot(distmat,observ_bray)

# observer dissimilarity x species dissimilarity
mantel(observ_bray,spec_bray)
plot(observ_bray,spec_bray)

# repeat 3 mantel tests with jaccard dissimilarities
spec_jaccard<-as.matrix(vegdist(SpecSampMatTrans,method="jaccard",diag=TRUE))
observ_jaccard<-as.matrix(vegdist(ObservSampMatTrans,method="jaccard",diag=TRUE))
mantel(km_dist,spec_jaccard)
mantel(km_dist,observ_jaccard)
mantel(observ_jaccard,spec_jaccard)

# mantel correlograms of species dissimilarity
# detach ncf package!
detach(package:ncf)

# species composition has significant autocorrelation at distances ranging from 10-20 km
spec.correlog<-mantel.correlog(spec_bray,distmat)
plot(spec.correlog)

# observer composition has significant autocorrelation at distances ranging from 10-20 km
observ.correlog<-mantel.correlog(observ_bray,distmat)
plot(observ.correlog)



#### Tom's additions end here  #####
